...
//#region node_modules/.nitro/vite/services/ssr/assets/catalog-CPtK0GY3.js
...
var getHomeFeed = createServerFn({ method: "GET" }).handler(getHomeFeed_createServerFn_handler, async () => {
	const yt = await import("./ytmusic.server-BUWalc_p.mjs").then((n) => n.n);
...
var getGenreMix = createServerFn({ method: "GET" }).validator((d) => d).handler(getGenreMix_createServerFn_handler, async ({ data }) => {
	return uniqueTracks(await (await import("./ytmusic.server-BUWalc_p.mjs").then((n) => n.n)).searchYtMusic(data.query, 30)).slice(0, 30);
});
...
var getVideoTrack = createServerFn({ method: "GET" }).validator((d) => d).handler(getVideoTrack_createServerFn_handler, async ({ data }) => {
	...
	if (!id) return null;
	const hits = await (await import("./ytmusic.server-BUWalc_p.mjs").then((n) => n.n)).searchYtMusic(id, 6).catch(() => []);
...
var getPlayUrl = createServerFn({ method: "GET" }).validator((d) => d).handler(getPlayUrl_createServerFn_handler, async ({ data }) => {
	...
	return { url: await (await import("./ytmusic.server-BUWalc_p.mjs").then((n) => n.n)).getAudioUrl(id) || null };
});
//#endregion
