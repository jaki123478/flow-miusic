...
import { d as getTrackLyrics, l as getRelatedTracks, p as searchCatalog, t as createMoodMix, u as getTopRadios } from "./lyrics-DWcjsahh.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/assistant-DdsC4pdG.js
var INTENTS = [
	"play",
	"queue",
	"search",
...
var chatTurn = createServerFn({ method: "POST" }).validator((d) => d).handler(chatTurn_createServerFn_handler, async ({ data }) => {
	...
	try {
		...
		if (intent === "lyrics") {
			...
			const lines = await getTrackLyrics({ data: {
				title,
				artist
			} });
			if (!lines.length) return result("Non ho trovato il testo di questo brano.", "lyrics");
			const snippet = lines.map((l) => l.text).filter(Boolean).slice(0, 8).join("\n");
